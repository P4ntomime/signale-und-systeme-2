...
\begin{minipage}[c]{0.36\columnwidth}
    Wenn $\bm{A_{diag}} = \bm{V}^{-1} \cdot A \cdot \bm{V}$ ist und $\lambda_i$ die Eigenwerte von $\bm{A}$ sind
\end{minipage}
...
\subsubsection{5 -- Definition der Reihenentwicklung}
...
\begin{minipage}[c]{0.48\columnwidth}
    ...
    = \begin{bmatrix*} a^k & 0 \\ \sum\limits_{l=0}^{k-1} a^{k-l-1}  & c^k \end{bmatrix*} $$
\end{minipage}


%TODO: Hier sollte nächste Vorlesung noch etwas kommen


\subsubsection{Eigenschaften der Fundamentalmatrix $\bm{\Phi(t)$}}
...
\myul{\textbf{Beobachtbarkeit der Zustände}}
...
\begin{itemize}
    \item Ein System ist \textbf{steuerbar}, wenn es für jeden Anfangszustand $\underline{x}_0$ und jeden Endzustand $\underline{x}_1$
        ...
        $\underline{x}_0$ zu $\underline{x}_1$ bringt, d.h. $\underline{x}(t_1) = \underline{x}_1$. % Definition Dubach
    \item Ein System ist \textbf{nicht steuerbar}, wenn es Zustände $\underline{x}(t)$ gibt, die nicht von den 
        Eingängen $\underline{u}(t)$ beeinflusst werden. % Skript 
\end{itemize}
...
\subsubsection{Diagonalform und Jordan-Normalform}{271-273}

\subsubsection{Jrodan-Normalform für mehrfache, reelle Pole}{274}