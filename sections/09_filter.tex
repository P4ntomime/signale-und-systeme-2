...
\begin{minipage}[c]{0.48\columnwidth}
    ...
    % TODO: formatting
    \begin{tabular}{c c c}
        $ \boxed{ S = \frac{s}{\omega_r}} $             & & $\boxed{ \Omega = \frac{\omega}{\omega_r} }$ \\
        \\
        $ \boxed{ \sigma' = \frac{\sigma}{\omega_r}}$   & & \\
    \end{tabular}
    \textbf{Hinweis:} Zur Entnormierung wird jeweisl $S$ in der normierter Funktion durch $\frac{s}{\omega_r}$ ersetzt.
\end{minipage}
...
Das die charakteristische Funktion wird bei der Butterworth-Approximation als \\
$K(\Omega^2) = (\Omega^2)^n = \Omega^{2n}$ gewählt.

\begin{minipage}[c]{0.48\columnwidth}
    \begin{center}
        \textbf{\myul{Allgemein}}
    \end{center}
    $$ | H(\jimg \Omega) |^2 = \frac{1}{1 + K(\Omega^2)} $$
\end{minipage}
\hfill
\begin{minipage}[c]{0.48\columnwidth}
    \begin{center}
        \textbf{\myul{Butterworth}}
    \end{center}
    $$\boxed{ |H(\jimg \Omega)| = \frac{1}{\sqrt{ 1 +\Omega^{2n}}} } $$
\end{minipage}

\textrightarrow\ $|H(\jimg \Omega)|$ ist streng-monoton fallend!
...
%TODO
% \includegraphics[width=\columnwidth]{images/filter_pollage_butterworth.png}
